                ...
                try
                {
                    ...
                    if (hostName != string.Empty)
                    {
                        ...
                        using (NetworkStream serverStream = new NetworkStream(server))
                        {
                            ...
                            int answerLength;
                            ReadFromStream(serverStream, out answer, out answerLength, Encoding.UTF8);
                            byte[] answerBytes = Encoding.UTF8.GetBytes(answer.ToString());

                            string[] head = answer.ToString().Split(SplitChars);
                            string ResponseCode = head[0].Substring(head[0].IndexOf(" ") + 1);
                            Console.WriteLine(string.Format("{0} {1}", hostName, ResponseCode));
                            */

                            //clientStream.Write(answerBytes, 0, answerBytes.Length);
